...
class ErrorResponseEntity(body: ErrorResponse) : ResponseEntity<ErrorResponse>(body, HttpStatus.valueOf(body.status)) {
    ...
    companion object {
        ...
        private fun clientError(httpStatus: HttpStatus = HttpStatus.BAD_REQUEST, message: String?, requestId: String) = ErrorResponseEntity(ErrorResponse(httpStatus, message, requestId))

        fun serverError(message: String?, requestId: String) = serverError(HttpStatus.INTERNAL_SERVER_ERROR, message, requestId)
        fun serverError(httpStatus: HttpStatus = HttpStatus.INTERNAL_SERVER_ERROR, message: String?, requestId: String) = ErrorResponseEntity(ErrorResponse(httpStatus, message, requestId))
    }
}